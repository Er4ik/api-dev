...
export class ApplicantService {
    ...
    async getAllApplicants(query: getAllApplQuery): Promise<object> {
        try {
            ...
            throw new HttpException(`Error get all applicants -> ${err}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getApplicantByID(id: string): Promise<object> {
        try {
            ...
            throw new HttpException(`Error get applicant by id -> ${err}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async updateAppl(body: applicantUpdateBody, id: string, header: object): Promise<void> {
        try {
            if(this.valid.checkValidBody(body)) {
                const dataVerify = this.verify.verifyToken(header); // доделать
                const bodyToDB = await this.helper.prepareBodyToAdd(body);
